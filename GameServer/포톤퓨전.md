...
권한이 있는 클라이언트 혹은 서버로부터 상태가 변경될 시 다른 모든 클라이언트로 상태가 복제된다.
...
서버에서 매 Tick마다 현재 상태를 모든 클라이언트에서 전파한다. 이 과정에서 클라이언트는 예측을 위해 FixedUpdateNetwork를 실행 후, 서버의 Tick 결과를 받고, 재실행을 할 수 있다.
...
* [Networked]   
애트리뷰트를 이용하면 NetworkTransform에서 Transform을 동기화시키듯 모든 클라이언트에서 프라퍼티 값을 일치시킬 수 있다. 이때 프라퍼티는 자동 생성된 프라퍼티({get;set;})여야 한다.
...
### 어떤 코드가 어디까지 영향을 미치는 건지 모르겠음...

-> 근본적으로 모르는 지점이 이거임.
HasAuthority 체크를 하는 경우 해당 상황에만 돌아감.
근데 RPC는 ? Network Value 변화는 ? 단순 함수는 ? NetworkBehaviour 함수는 ?

...
테스트하면서알아내자...

어차피 코드 실행은 전부 라이프 사이클에서 실행됨.
인풋 - 실행이 분리된 이상 전부 흐름 제어를 내가 한다고 보면 될 듯. ^^ 감이 좀 잡힘.
